...
"""
    response = client.chat.completions.create(model="gpt-5", messages=[{"role": "user", "content": prompt}], temperature=0)
...
def build_stage1_ap_with_tavily(product: str, status_container):
    ...
    response = client.chat.completions.create(model="gpt-5", messages=[{"role": "user", "content": intro_prompt}], temperature=0)
...
"""
    response = client.chat.completions.create(model="gpt-5", messages=[{"role": "system", "content": SYSTEM_PROMPT}, {"role": "user", "content": prompt}], temperature=0)
    return response.choices[0].message.content.strip()
