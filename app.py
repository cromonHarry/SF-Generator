...
import time
import sys
from ap_model_visualization import collapsible_visualization
...
def navigate_to_visualization():
    if 'bg_history' not in st.session_state or not st.session_state.bg_history:
        st.warning("请先生成一个故事，再进行AP模型可视化。")
    else:
        st.session_state.page = 'visualization'


def navigate_to_generator():
    st.session_state.page = 'generator'
...
if st.session_state.page == 'generator':
    ...
    if generate_button and product and user_experience and avant_garde_issue and uploaded_file and not st.session_state.generated:
        ...
        with final_container:
            ...
            # Save background_history.json for AP model visualization
            with open("background_history.json", "w", encoding="utf-8") as f:
                json.dump(bg_history, f, ensure_ascii=False, indent=2)
    ...
    if st.session_state.generated:
        ...
        # Add visualization button and reset button
        col1, col2 = st.columns(2)
        with col1:
            # Button to view AP model visualization
            st.button("📊 View AP Model Visualization", on_click=navigate_to_visualization, type="primary")
        
        with col2:
            # Reset button
            if st.button("🔄 Generate New Story", type="secondary"):
                # Reset session state
                st.session_state.generated = False
                st.session_state.bg_history = []
                st.session_state.description_history = []
